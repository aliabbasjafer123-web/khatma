...
function renderForms() {
  ...
  grid.innerHTML = formsToShow.map(f => {
    ...
    return `
      <div class="form-card status-${status}" onclick="showFormDetails(${f.id})">
  ...
  document.getElementById('formDetailsContent').innerHTML = `
    <div style="background:#fff; border-radius:12px; padding:20px; box-shadow:0 2px 8px rgba(0,0,0,0.05); margin-bottom:20px;">
      ...
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; border-bottom: 2px solid #f0f0f0; padding-bottom:15px;">
        <div style="font-size:22px; font-weight:900; color:var(--primary)">\u{1F4D6} الاستمارة رقم <span style="color:#8b0000;">${formId}</span></div>
        <div>${statusHtml}</div>
      </div>
      
      <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-bottom:20px;">
        <div style="background:#f8f9fa; padding:15px; border-radius:8px; text-align:center; border:1px solid #e9ecef;">
          <div style="color:#6c757d; font-size:13px; font-weight:bold; margin-bottom:5px;">من صفحة</div>
          <div style="font-size:24px; font-weight:900; color:var(--primary);">${form.startPage}</div>
        </div>
        <div style="background:#f8f9fa; padding:15px; border-radius:8px; text-align:center; border:1px solid #e9ecef;">
          <div style="color:#6c757d; font-size:13px; font-weight:bold; margin-bottom:5px;">إلى صفحة</div>
          <div style="font-size:24px; font-weight:900; color:var(--primary);">${form.endPage}</div>
        </div>
      </div>
...
`;
  ...
  saveFormEditBtn.onclick = () => {
    ...
    if (!newParticipantId) {
      Toast.error('��لرجاء اختيار المشترك لحفظ التوزيع');
      return;
    }
